...
const CreateGame: React.FC<gameProps> = ({ game_size, userKey }) => {
    ...
    return (
        <form className="flex flex-col h-fit w-[21vw] gap-2.5 p-5 border border-[#272B30] rounded-[10px] text-white bg-black shadow-[rgba(0,0,0,0.05)_0px_1px_2px_0px] font-[Terminus]" onSubmit={handleSubmit}>
          <h2 className="mb-[0.2vw]">Game Settings</h2>
          <label>
          <span className='mb-[5px] ml-[10px]'>Name</span>
            <input type="text" className='border border-[#272B30] rounded-[5px] w-full h-[30px] bg-black text-white pl-1 font-[Terminus]' name="game_name" value={String(formData[5])} onChange={handleChange(5)} />
          </label>
          <label>
          <span className='mb-[5px] ml-[10px]'>Max Buy-In</span>
            <input className="no-arrows border border-[#272B30] rounded-[5px] w-full h-[30px] bg-black text-white pl-1 font-[Terminus]" type="number" name="buy_in_min" value={String(formData[1])} onChange={handleChange(1)} />
          </label>
          <label>
          <span className='mb-[5px] ml-[10px]'>Min Buy-In</span>
            <input className="no-arrows border border-[#272B30] rounded-[5px] w-full h-[30px] bg-black text-white pl-1 font-[Terminus]" type="number" name="buy_in_max" value={String(formData[2])} onChange={handleChange(2)} />
          </label>
          <label>
          <span className='mb-[5px] ml-[10px]'>Token (mint address)</span>
            <input className="no-arrows border border-[#272B30] rounded-[5px] w-full h-[30px] bg-black text-white pl-1 font-[Terminus]" type="text" name="game_token" value={String(formData[4])} onChange={handleChange(4)} />
          </label>
