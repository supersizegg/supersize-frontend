...
const GameComponent: React.FC<GameComponentProps> = ({
    ...
    const loop = (time: number) => {
        ...
        while (accumulator >= timeStep) {
            ...
            if (currentPlayerPos) {
                currentPlayerPos = updatePlayerPosition(
                    currentPlayerPos,
                    currentPlayerPos.target_x,
                    currentPlayerPos.target_y,
                    timeStep,
                );
            }
    ...
    const updatePlayerPosition = (
        ...
        const delta_y =
            (player.speed * scale_up * Math.sin(deg) * (dt / 1000)) / slow_down;
        const delta_x =
            (player.speed * scale_up * Math.cos(deg) * (dt / 1000)) / slow_down;
